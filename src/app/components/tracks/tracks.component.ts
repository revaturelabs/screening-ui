...
export class TracksComponent implements OnInit {
  ...
  getAssociated() {
    for (let i = 0; i < this.allCategories.length; i++) {
      if (this.checkContains(this.allCategories[i])) {
        if (
          !this.singleTrackCategoryIds.includes(
            this.allCategories[i].categoryId
          )
        ) {
    ...
    if (this.singleTrack) {
      for (let i = 0; i < this.allWeights.length; i++) {
        if (this.allWeights[i].track.title === this.singleTrack.title) {
          if (
            this.allWeights[i].category.categoryDescription ===
  ...
  updateTrack(modal: Track) {
    ...
    if (
      this.categoryWeightSum === 100 ||
      this.categoriesAndWeights.length === 0
    ) {
      ...
      this.trackService
        .updateTrackCategories(this.track, categoriesId, weights)
        .subscribe(results => {
          this.grabAllTracks();
        });
