...
export class SearchPipe implements PipeTransform {
  ...
  transform(items: ScheduledScreening[], searchText: string): any[] {
    ...
    return items.filter(it => {
      ...
      return it.candidate.name.toLowerCase().includes(searchText) || it.candidate.name.toLowerCase().includes(searchText)
      || (it.candidate.name.toLowerCase() + ' ' + it.candidate.name.toLowerCase()).includes(searchText);
    });
  }
}
